...
import CommandFleetDeck from '@/components/dashboard/command-fleet-deck';
...
async function TimelineSection() { const { reservations, vehicles: rawVehicles } = await getDashboardData(); return <GanttChart reservations={reservations} vehicles={rawVehicles.map((vehicle: any) => ({ id: vehicle.id, model: vehicleLabel(vehicle), license_plate: vehicle.license_plate }))} />; }
async function FleetDeckSection() { const { vehicles } = await getDashboardData(); return <CommandFleetDeck vehicles={vehicles} />; }
