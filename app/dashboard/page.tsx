...
async function StatusSection() { const [reservations, vehicles] = await Promise.all([getReservations(), getVehicles()]); return <VehicleStatusCards vehicles={vehicles} reservations={reservations} />; }
async function TimelineSection() { const [reservations, rawVehicles] = await Promise.all([getReservations(), getVehicles()]); return <GanttChart reservations={reservations} vehicles={rawVehicles.map(vehicle => ({ id: vehicle.id, model: typeof vehicle.model === 'string' ? vehicle.model : (vehicle.model as any)?.name || 'Veículo sem modelo', license_plate: vehicle.license_plate }))} />; }
async function QrSection() { return <QrDispatchPanel vehicles={await getVehicleQrPreviews()} />; }
...
async function ReservationSnapshot() { const reservations = await getReservations(); return <section className="ops-panel p-5"><div className="flex items-center justify-between border-b border-[#1c2637] pb-3"><div><h2 className="text-sm font-extrabold uppercase text-white">Reservas</h2><p className="mt-1 text-xs text-slate-400">Próximas alocações</p></div><Link href="/dashboard/reservations" className="text-xs font-bold text-red-300">Ver todas</Link></div>{reservations.length ? <div className="mt-3 space-y-2">{reservations.slice(0, 3).map(item => <Link key={item.id} href="/dashboard/reservations" className="block rounded-lg border border-[#334460] bg-[#182232] p-3 hover:bg-[#1e2b3e]"><p className="truncate text-sm font-bold text-slate-100">{item.purpose?.split(' - ')[0] || 'Reserva'}</p><p className="mt-1 text-xs text-slate-400">{item.driver_name || item.driver?.name || 'Condutor não informado'}</p></Link>)}</div> : <p className="py-5 text-center text-sm text-slate-400">Nenhuma reserva ativa.</p>}</section>; }
