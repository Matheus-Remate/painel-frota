...
import FleetSearch from '@/components/dashboard/fleet-search';
...
export default function DashboardHeader({ user, initialNotifications }: HeaderProps) {
    ...
    useEffect(() => {
        const close = (event: MouseEvent) => { if (notificationRoot.current && !notificationRoot.current.contains(event.target as Node)) setNotificationsOpen(false); };
    ...
    return <header className="sticky top-0 z-50 border-b border-[#232f45] bg-[#131a26]/95 backdrop-blur-md">
        <div className="mx-auto max-w-[1920px] px-4 sm:px-6 lg:px-8">
            <div className="flex h-16 items-center gap-3">
                ...
                <div ref={notificationRoot} className="relative"><button onClick={toggleNotifications} aria-label="Notificações" aria-expanded={notificationsOpen} className="relative rounded-lg border border-[#26344d] p-2 text-slate-300 hover:bg-[#182232] hover:text-white"><Bell className="h-5 w-5" />{unread > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#990000] px-1 text-[10px] font-bold text-white ring-2 ring-[#131a26]">{unread > 9 ? '9+' : unread}</span>}</button>{notificationsOpen && <div className="absolute right-0 mt-2 max-h-96 w-80 overflow-y-auto rounded-xl border border-[#334460] bg-[#141e2e] shadow-2xl"><div className="border-b border-[#26344d] px-4 py-3"><p className="text-sm font-bold text-white">Central de alertas</p><p className="text-xs text-slate-400">Eventos que exigem atenção</p></div>{notifications.length ? notifications.map(item => <Link key={item.id} href={item.href} onClick={() => setNotificationsOpen(false)} className="block border-b border-[#26344d]/70 px-4 py-3 hover:bg-[#1e2b3e]"><p className="text-sm font-medium text-slate-100">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.message}</p></Link>) : <p className="p-6 text-center text-sm text-slate-400">Nenhum alerta.</p>}</div>}</div>
