...
export default function EmergencyCheckoutForm({ vehicleId, token }: { vehicleId: string; token: string }) {
    ...
    async function submit(formData: FormData) { setLoading(true); setError(null); const result = await registerEmergencyCheckout(formData); if (!result.success) { setError(result.error || 'Não foi possível registrar a retirada.'); setLoading(false); return; } router.replace(`/mobile/vehicle/${vehicleId}?token=${encodeURIComponent(token)}&checkout=success`); router.refresh(); }
